...
export const filterByDateYouTube = (videos: YouTubeVideoResult[], dateLimit: Date) => {
...
export const filterByDurationYouTube = (videos: YouTubeVideoResult[], minimumDurationInSeconds = 0, maximumDurationInSeconds = 180000) => {
  const filteredVideos = videos.filter((video) => {
    return (convertISOToSeconds(video.contentDetails.duration) > minimumDurationInSeconds) && (convertISOToSeconds(video.contentDetails.duration) < maximumDurationInSeconds);
  });
...
export const filterByKeywordYouTube = (videos: YouTubeVideoResult[], keyword: string) => {
  const filteredVideos = videos.filter((video) => {
    return video.snippet.title.toLowerCase().includes(keyword.trim().toLowerCase());
  });
