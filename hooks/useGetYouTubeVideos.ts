...
export const useGetYouTubeVideos = (uploadsPlaylistId: string, conditions?: boolean) => {
  ...
  const { isLoading, isError, data, error } = useQuery(['youtubeChannelVideos', uploadsPlaylistId], async () => {
    ...
    const playListResult = playlistResponse.result as YouTubePlaylistItemListResponse;
